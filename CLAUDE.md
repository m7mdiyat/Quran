...
Custom fonts MUST be bundled locally — never fetched remotely at runtime — or a FRESH install flashes the fallback then swaps (FOUT), and an OFFLINE launch renders the wrong (system) font entirely. Weights used only *later* (e.g. the panel fonts) must ALSO be force-loaded at boot via `document.fonts.load()` so the first sheet that uses them doesn't flash; fonts used on first paint don't need that. Don't assume a font is "already cached from elsewhere."
...
Amiri (the Arabic fallback after MeQuran) and Outfit (the Latin / UI font) are likewise bundled locally — the EXACT subsets Google Fonts serves ship as `public/fonts/{amiri,outfit}-<weight>-<subset>.woff2` with a local `@font-face` block in `index.html` inside `<style id="heroFonts">`. This replaced the old render-blocking `fonts.googleapis.com` stylesheet (and its now-removed `googleapis`/`gstatic` preconnects), which on an offline cold start blocked first paint then fell back to system fonts. `unicode-range` is preserved verbatim, so the browser still fetches only the subset a glyph needs. Unlike IBM Plex these are used on FIRST PAINT, so `font-display:swap` decodes them immediately — no `document.fonts.load()` force-load needed. Outfit weight 300 was dropped (zero uses in the CSS). **Net result: the app now loads ZERO remote fonts** — MeQuran (`/MeQuran.ttf`, preloaded), IBM Plex, Amiri and Outfit are all local, so typography is instant and fully offline on a fresh install. To add an Amiri/Outfit weight: re-fetch its woff2 subsets from the Google Fonts CSS (request with a desktop-Chrome UA so it returns woff2 URLs), drop them in `public/fonts/`, and add matching `@font-face` rules (with their `unicode-range`) to the `heroFonts` block.
