...
import { buildReciterPickerHtml, initReciterPicker } from "./reciter-picker.js";
...
// Rotating "preparing" status lines. Plain text ONLY — these are rendered into
// a text node (swapText → textContent), so any embedded markup would show as
// raw escaped text. Never put an <svg>/HTML here.
const TAFSIR_DL_MESSAGES = [
  "جارٍ تجهيز ميزة التفسير",
  "نُجهّز لك كنوز التفسير",
  "جارٍ جمع أقوال المفسّرين",
  `جارٍ تجهيز ميزة "مختصر التفاسير"`,
];
...
const RECITERS = {
  ...
  // Full-surah engine only (no per-ayah files): gapless MP3 + Format-A timings
  // on GCS, sourced Hafs/Murattal (abdulbasit/husary/minshawi from quran.com's
  // matched audio+segments; maher built from EveryAyah per-ayah concatenation).
  abdulbasit: { name: 'عبد الباسط عبد الصمد', path: 'abdulbasit', color: 'abdulbasit', engineOnly: true },
  husary: { name: 'محمود خليل الحصري', path: 'husary', color: 'husary', engineOnly: true },
  minshawi: { name: 'محمد صديق المنشاوي', path: 'minshawi', color: 'minshawi', engineOnly: true },
  maher: { name: 'ماهر المعيقلي', path: 'maher', color: 'maher', engineOnly: true }
};
const RECITER_ORDER = ['alijaber', 'shuraim', 'ayoub', 'qasim', 'dosari', 'luhaidan', 'abdulbasit', 'husary', 'minshawi', 'maher'];
...
function isOfflineAudioError(err) {
  ...
  // A fetch that got an HTTP RESPONSE (e.g. a missing timings file → 404, or a
  // 5xx) proves the network is up — a data/server problem, NOT offline. So
...
function buildTafsirReciterChips() {
  document.querySelectorAll('[data-tafsir-settings="reciter"]').forEach(row => {
    ...
    row.innerHTML = buildReciterPickerHtml(RECITER_ORDER, RECITERS, CURRENT_RECITER);
  });
}
...
// One document-level delegate drives the reciter "plus → menu" morph across
// Tafsir, Mushaf, and fullscreen (idempotent).
initReciterPicker();
